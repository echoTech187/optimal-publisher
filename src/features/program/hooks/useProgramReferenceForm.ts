...
import { fetchMajors, fetchBookTitles, fetchBookTopics } from '@/features/form/data';
...
export const useProgramReferenceForm = () => {
    ...
    const handleMajorChange = useCallback(async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (majorId) {
            try {
                setLoading(true);
                const bookTitleData = await fetchBookTitles(majorId);
    ...
    const handleBookTitleChange = useCallback(async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (bookTitleId) {
            try {
                setLoading(true);
                const bookTopicData = await fetchBookTopics(bookTitleId);
